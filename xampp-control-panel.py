...
class MainWindow(SimpleGladeApp):
    ...
    def get_xampp_path(self):
        ...
        return self.CONFIG_FILE.get('main', 'xampp_path')

    def set_xampp_path(self, path_to_xampp):
        """ set the path info in the config file """
        self.CONFIG_FILE.set('main', 'xampp_path', path_to_xampp)
...
class PrefsWindow(SimpleGladeApp):
    ...
    def on_TextBox_changed(self, widget, *args):
        self.modified = True
    ...
    def on_browseRootButton_clicked(self, widget, *args):
        """ handler for file chooser - select path to XAMPP installation """
        fc = gtk.FileChooserDialog(
            title = 'Select the Webroot directory',
            action = gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER,
            buttons = (gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK)
        )
        fc.set_filename(self.rootTextBox.get_text())
        response = fc.run()
        if response == gtk.RESPONSE_OK:
            self.rootTextBox.set_text(fc.get_filename())
        fc.destroy()

    def on_applyButton_clicked(self, widget, *args):
        if self.modified:
            xampppath = self.pathTextBox.get_text()
            self.parent.CONFIG_FILE.set('main', 'xampp_path', xampppath)
            self.set_SQL_Setting()
            self.set_Apache_Setting()

            # Some value has been changed, so we need to restart the services
            os.popen(self.parent.get_xampp_command("stop"))

            #Don't start if there is no path
            if xampppath != "":
                os.popen(self.parent.get_xampp_command("start"))

        self.PrefsWindow.destroy()

    def on_defaultDbPortButton_clicked(self, widget, *args):
        self.dbportTextBox.set_text('3306')

    def on_defaultWebPortButton_clicked(self, widget, *args):
        self.webportTextBox.set_text('80')

    def on_cancelButton_clicked(self, widget, *args):
        print "on_cancelButton_clicked called with self.%s" % widget.get_name()
        self.PrefsWindow.destroy()

    def get_Apache_Setting(self):
        with open(self.HTTPD) as myFile:
            for num, line in enumerate(myFile, 1):
                if re.match('DocumentRoot "(.*?)"', line):
                    rootDir = line.split(' ', 1)
                    self.rootTextBox.set_text(rootDir[1].strip()[1:-1])
                    self.apRootDir = rootDir[1].strip()[1:-1]
                if re.match('Listen ([0-9]{2,5})', line):
                    port = line.split(' ', 1)
                    self.webportTextBox.set_text(port[1].strip())
                    self.apPort = port[1].strip()

    def set_Apache_Setting(self):
        port = self.webportTextBox.get_text()
        rootDir = self.rootTextBox.get_text()
        serverRoot = self.pathTextBox.get_text()
        shutil.copyfile(self.HTTPD, self.BACKUP)
        conf = open(self.HTTPD).read()

        # Set the DocumentRoot
        print 'Write apache documentroot'
        conf = re.sub('DocumentRoot "(.*?)"',
                         'DocumentRoot "' + rootDir + '"',
                          conf)
        conf = re.sub('<Directory "' + self.apRootDir + '">',
                         '<Directory "' + rootDir + '">',
                          conf)
        # Set the port
        print 'Write Apache port'
        conf = re.sub('Listen ([0-9]{2,5})',
                         'Listen ' + port,
                          conf)
        open(self.HTTPD, 'w').write(conf)

        # Set the serverroot
        print 'Write Apache serverroot'
        conf = re.sub('ServerRoot "(.*?)"',
                         'ServerRoot "' + serverRoot + '"',
                          conf)
        open(self.HTTPD, 'w').write(conf)

    def get_SQL_Setting(self):
        self.dbportTextBox.set_text(self.sqlFile.get('client', 'port'))

    def set_SQL_Setting(self):
        print 'Write sql port'
        self.sqlFile.set('client', 'port', self.dbportTextBox.get_text())
        self.sqlFile.set('mysqld', 'port', self.dbportTextBox.get_text())
